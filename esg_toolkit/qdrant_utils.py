...
class QdrantDBWrapper:
    ...
    def upsert(self, collection_name: str, data: pd.DataFrame, embeddings: list):
        ...
        self.collection = self.use_collection(
            collection_name=collection_name, size=len(embeddings[0])
        )
        row_num = self.count(collection_name=collection_name, count_filter=None).count
        all_data = (
            self.scroll(
                collection_name=collection_name, limit=row_num, scroll_filter=None
            )
            if row_num
            else None
        )
        ...
        if not _data.empty and len(_data) == len(_embeddings):
            self.client.upsert(
                ...
                points=models.Batch(
                    ...
                    vectors=_embeddings,
                    payloads=_data.drop(["idx", "temp_vector"], axis=1).to_dict(
                        "records"
                    ),
                ),
            )
    ...
    def detect_broken_data(self, collection_name: str):
        ...
        all_data = self.scroll(
            collection_name=collection_name, limit=count, scroll_filter=None
        )[0]
        broken_ids = []
